...
const Input = forwardRef(
  ({ label, error, helperText, type = "text", className = "", isTextArea = false, rows = 4, ...props }, ref) => {
    const inputStyles = `w-full px-5 py-3.5 rounded-xl border bg-[var(--color-surface-elevated)] text-[var(--color-text-main)] placeholder-[var(--color-text-muted)] focus:outline-none focus:ring-1 focus:ring-[var(--color-accent)] focus:border-[var(--color-accent)] transition-all ${
      error
